    ...
    def __str__(self):
        ...
        for field in reversed(field_ranges):
            if field[1] < (index + 1) * self.fieldsize:
                ...
                field_lines[index].append(
                    (field[0], (index + 1) * self.fieldsize - 1, field[2])
                )
                remain = (
                    field[1] - field[2] - ((index + 1) * self.fieldsize - 1 - field[2])
                )
                for i in range(remain // self.fieldsize):
                    index += 1
                    field_lines[index].append(
                        (
                            field[0],
                            (index + 1) * self.fieldsize - 1,
                            (index) * self.fieldsize,
                        ),
                    )
                    remain -= self.fieldsize
                if remain > 0:
                    index += 1
                    field_lines[index].append(
                        (field[0], field[1], index * self.fieldsize)
                    )
            else:
                index += 1
                field_lines[index].append(field)
